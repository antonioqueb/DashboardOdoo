...
export class GerenciaContabilidad extends Component {
  setup(){
        this.state = useState({
            quotations: {
                value:10,
...
async getOrders(){
  ...
  const prev_data = await this.orm.searchCount("sale.order", prev_domain)
  const percentage = ((data - prev_data)/prev_data) * 100
  //this.state.quotatioccns.percentage = percentage.toFixed(2) ok?
...
GerenciaContabilidad.template = "TobaccoMetricsPro.GerenciaContabilidad";
GerenciaContabilidad.components = { MetricCard, ChartRenderer};


// Log to console when adding to the registry to confirm it's being executed  ... 
console.log("Adding Dashboard to the registry");

registry.category("actions").add("TobaccoMetricsPro.GerenciaContabilidad", GerenciaContabilidad);